...
class Ctx:
    def __init__(self, episode: str, rights_basis: str) -> None:
        ...
        from nalu_qa_common import QaPaths
        self.contract_path = QaPaths(episode).contract
...
def lock_wardrobe(ctx: Ctx, row: dict[str, Any]) -> dict[str, Any]:
    ...
    reuse_note = None
    if item is None and plate:
        owner_row = ctx.library.get('assets', {}).get('characters', {}).get(owner, {})
        qa = owner_row.get('qa') or {}
        review_path = Path(qa.get('review_file') or '/nonexistent')
        reuse = owner_row.get('reuse_evidence') or {}
        if (owner_row.get('reuse_review', {}).get('decision') == 'ACCEPT_REUSE'
                and reuse.get('source_asset_id') and review_path.is_file()
                and sha256_file(review_path) == qa.get('review_file_sha256')):
            review = load_json(review_path)
            prior = next((x for x in review.get('items', [])
                          if x.get('item_id') == reuse['source_asset_id']), None)
            reviewed_shas = {m.get('sha256') for m in (prior or {}).get('request_item', {}).get('media', [])}
            if (review.get('validation', {}).get('status') == 'PASS'
                    and (prior or {}).get('verdict') in {'PASS', 'PASS_WITH_P2'}
                    and plate['sha256'] in reviewed_shas):
                answers = prior.get('answers') or {}
                reuse_note = {'kind': 'SHA_BOUND_HISTORICAL_WARDROBE_REVIEW',
                              'review_file': str(review_path), 'review_sha256': qa['review_file_sha256'],
                              'source_asset_id': reuse['source_asset_id'],
                              'plate_sha256': plate['sha256'],
                              'current_reuse_review': owner_row['reuse_review']}
    if item is None and plate and ctx.runtime_library and reuse_note is None:
...
def accent_locale(spec: dict[str, Any]) -> str | None:
    """Accept either declared field, but never silently resolve a conflict."""
    values = {spec[key] for key in ('locale', 'language') if spec.get(key)}
    return next(iter(values)) if len(values) == 1 else None


def declared_scene_room(gsm: dict[str, Any], location: str,
                        scene_ids: set[str]) -> tuple[str | None, str | None]:
    candidates = set()
    for space in gsm.get('space_maps', []):
        rooms = {r.get('room_id') for r in space.get('rooms', [])}
        for mapping in space.get('scene_mappings', []):
            if (mapping.get('location_id') == location
                    and mapping.get('scene_id') in scene_ids
                    and mapping.get('room_id') in rooms):
                candidates.add((space['global_space_map_id'], mapping['room_id']))
    # Ambiguous locations require an authored decision, never last-row wins.
...
def lock_scene(ctx: Ctx, row: dict[str, Any]) -> dict[str, Any]:
    ...
    if not room_id:
        map_id, room_id = declared_scene_room(ctx.gsm, loc, {
            state['scene_id'] for state in spec.get('scene_states', []) if state.get('scene_id')})
...
def lock_declared_audio(ctx: Ctx, category: str, row: dict[str, Any]) -> dict[str, Any]:
    ...
    if category == "ambience":
        ...
        locale = accent_locale(spec)
        lock.update({"locale": locale, "pronunciation_profile": spec.get("pronunciation_profile"),
                     "forbidden": spec.get("forbidden")})
        checks.append({"id": "locale_zh_cn", "status": "PASS" if locale == "zh-CN" else "FAIL",
                       "declared_locale": spec.get("locale"), "declared_language": spec.get("language")})
