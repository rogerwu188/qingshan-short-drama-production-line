#!/usr/bin/env python3
"""Fill and submit an E08 action_role review from the keyframe findings (claude-code-nalu-vlm).
Overrides: /tmp/e08_action_role_overrides.json {unit_id: {answers:{}, observation:"", observed:{}, defects:[]}}"""
...
req = sorted(glob.glob(f"{_np.RUNTIME_ROOT}/runtime/reviews/E08/E08-action_role-*_request.json"))[-1]
...
F = json.load(open("/tmp/e08_q1_findings.json"))["items"]
OV = json.load(open("/tmp/e08_action_role_overrides.json")) if os.path.exists("/tmp/e08_action_role_overrides.json") else {}
