#!/usr/bin/env python3
"""Fill and submit an E05 start_frame review from the keyframe findings (claude-code-nalu-vlm).
Each item is a unit; its start frame is the unit's first-shot keyframe already viewed for Q1.
Extra per-unit overrides: /tmp/e05_start_frame_overrides.json {unit_id: {answers:{}, observation:"", ...}}"""
...
req = sorted(glob.glob(f"{_np.RUNTIME_ROOT}/runtime/reviews/E05/E05-start_frame-*_request.json"))[-1]
VENV = f"{_np.VENV_PYTHON}"; PROTO = f"{_np.TOOLS_DIR}/vlm_review_protocol.py"
tpl = "/tmp/e05_sf_answers_tpl.json"
...
OV = json.load(open("/tmp/e05_start_frame_overrides.json")) if os.path.exists("/tmp/e05_start_frame_overrides.json") else {}
...
out = "/tmp/e05_sf_answers_filled.json"; json.dump(ans, open(out, "w"), ensure_ascii=False, indent=2)
