#!/usr/bin/env python3
"""Fill and submit the E05 video_q2 review (claude-code-nalu-vlm) from /tmp/e05_q2_findings.json written after viewing the Q2 sheets:
/tmp/e05_q2/sheet_*.png (5 original-resolution frames per unit, 4 units per sheet) + single frames opened for
...
req = sorted(glob.glob(f"{_np.RUNTIME_ROOT}/runtime/reviews/E05/E05-video_q2-*_request.json"))[-1]
VENV = f"{_np.VENV_PYTHON}"; PROTO = f"{_np.TOOLS_DIR}/vlm_review_protocol.py"
tpl = "/tmp/e05_q2_tpl.json"
...
F = json.load(open("/tmp/e05_q2_findings.json")); missing = []
...
out="/tmp/e05_q2_answers_filled.json"; json.dump(ans,open(out,"w"),ensure_ascii=False,indent=2)
