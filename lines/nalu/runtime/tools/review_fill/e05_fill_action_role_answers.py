#!/usr/bin/env python3
"""Fill and submit an E05 action_role review from the keyframe findings (claude-code-nalu-vlm).
Overrides: /tmp/e05_action_role_overrides.json {unit_id: {answers:{}, observation:"", observed:{}, defects:[]}}"""
...
req = sorted(glob.glob(f"{_np.RUNTIME_ROOT}/runtime/reviews/E05/E05-action_role-*_request.json"))[-1]
VENV = f"{_np.VENV_PYTHON}"; PROTO = f"{_np.TOOLS_DIR}/vlm_review_protocol.py"
tpl = "/tmp/e05_ar_answers_tpl.json"
...
F = json.load(open("/tmp/e05_keyframe_findings.json"))["items"]
OV = json.load(open("/tmp/e05_action_role_overrides.json")) if os.path.exists("/tmp/e05_action_role_overrides.json") else {}
...
for item in ans["items"]:
    ...
    creature_ids = {"PROP-MUTANT-BEAST", "PROP-PURPLE-EYED-CROW"}
...
out = "/tmp/e05_ar_answers_filled.json"; json.dump(ans, open(out, "w"), ensure_ascii=False, indent=2)
