#!/usr/bin/env python3
"""Fill and submit an E06 action_role review from the keyframe findings (claude-code-nalu-vlm).
Overrides: /tmp/e06_action_role_overrides.json {unit_id: {answers:{}, observation:"", observed:{}, defects:[]}}"""
...
req = sorted(glob.glob(f"{_np.RUNTIME_ROOT}/runtime/reviews/E06/E06-action_role-*_request.json"))[-1]
VENV = f"{_np.VENV_PYTHON}"; PROTO = f"{_np.TOOLS_DIR}/vlm_review_protocol.py"
tpl = "/tmp/e06_ar_answers_tpl.json"
...
F = json.load(open("/tmp/e06_keyframe_findings.json"))["items"]
OV = json.load(open("/tmp/e06_action_role_overrides.json")) if os.path.exists("/tmp/e06_action_role_overrides.json") else {}
...
out = "/tmp/e06_ar_answers_filled.json"; json.dump(ans, open(out, "w"), ensure_ascii=False, indent=2)
