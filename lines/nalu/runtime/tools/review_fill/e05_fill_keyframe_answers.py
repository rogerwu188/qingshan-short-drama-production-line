#!/usr/bin/env python3
"""Fill and submit a keyframe (Q1) review for E05 from a findings file the reviewer wrote after
actually viewing the contact sheets (claude-code-nalu-vlm).

findings.json: {"default": {"answers": {...optional overrides...}},
                "items": {"E05-S01-01": {"verdict": "PASS"|"REJECT", "observation": "...",
                                          "answers": {q: v, ...}, "defects": [...],
...
a = ap.parse_args()
req = a.request or sorted(glob.glob(f"{_np.RUNTIME_ROOT}/runtime/reviews/E05/E05-keyframe-*_request.json"))[-1]
VENV = f"{_np.VENV_PYTHON}"; PROTO = f"{_np.TOOLS_DIR}/vlm_review_protocol.py"
tpl = "/tmp/e05_keyframe_answers_tpl.json"
...
out = "/tmp/e05_keyframe_answers_filled.json"; json.dump(ans, open(out, "w"), ensure_ascii=False, indent=2)
