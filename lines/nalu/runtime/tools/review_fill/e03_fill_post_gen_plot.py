#!/usr/bin/env python3
"""Fill and submit the E03 post_gen_plot review from /tmp/e03_pgp_findings.json (viewed 2 fps contact sheets)
and /tmp/e03_asr.json (local faster-whisper transcripts, zh)."""
...
req = sorted(glob.glob(f"{_np.RUNTIME_ROOT}/runtime/reviews/E03/E03-post_gen_plot-*_request.json"))[-1]
VENV = f"{_np.VENV_PYTHON}"; PROTO = f"{_np.TOOLS_DIR}/vlm_review_protocol.py"
tpl = "/tmp/e03_pgp_tpl.json"
...
F = json.load(open("/tmp/e03_pgp_findings.json")); ASR = json.load(open("/tmp/e03_asr.json"))
...
out = "/tmp/e03_pgp_answers_filled.json"; json.dump(ans, open(out, "w"), ensure_ascii=False, indent=2)
