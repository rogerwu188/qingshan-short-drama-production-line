#!/usr/bin/env python3
"""Fix V7's opening half-second freeze by starting wrist descent on frame 1."""
...
OUT_DIR = ROOT / "workflow/claude_writer_agent/production/e40_claude_writer_v3_140d4b7b_20260808/u02_v8_opening_overlap_deterministic_v1"
VIDEO = OUT_DIR / "E40_U02_V8_OPENING_OVERLAP_DETERMINISTIC_CANDIDATE_V1.mp4"
SPEC = OUT_DIR / "E40_U02_V8_OPENING_OVERLAP_DETERMINISTIC_SPEC_V1.json"
CONTACT = ROOT / "qa/e40_production_20260814/u02_v8_opening_overlap_deterministic_v1/E40_U02_V8_CONTACT_SHEET_V1.png"
...
def frame_at(base: np.ndarray, index: int, xx: np.ndarray, yy: np.ndarray) -> np.ndarray:
    ...
    map_x += curtain_weight * (
        ...
        + 10.0 * breath * np.sin((yy / 88.0) + 1.25)
    )
...
def main() -> None:
    ...
    if not any(row.get("id") == "PF-036" for row in memory.get("rules", [])):
        raise SystemExit("PF-036 must be durable before V8")
    ...
    payload.update({
        "schema": "qingshan.e40.u02.v8.opening_overlap_deterministic_spec.v1",
        "variant": "V8", "created_at": "2026-08-14T07:04:00Z",
        "failure_memory": {"path": str(MEMORY.relative_to(ROOT)), "sha256": sha(MEMORY), "rule_id": "PF-036"},
        "material_change_from_v7": "Wrist descent now starts on frame1 and overlaps fan compression; later turn begins at1.30s and curtain breath at2.50s so no half-second handoff gap remains.",
        "beats": [
            {"window": "0.00-0.50", "action": "fan compression plus simultaneous same-wrist descent"},
            {"window": "0.00-1.30", "action": "same wrist/fan/sleeve descends 48px and moves inward20px"},
            {"window": "1.30-2.50", "action": "same wrist turns inward6 degrees"},
            {"window": "0.00-4.00", "action": "continuous non-looping curtain life with hem pinned"},
            {"window": "2.50-4.00", "action": "one localized curtain breath"},
        ],
    })
    base.atomic_json(SPEC, payload)
    print(json.dumps({"status": "PASS_V8_RENDERED", "video": str(VIDEO.relative_to(ROOT)), "video_sha256": sha(VIDEO), "spec_sha256": sha(SPEC), "contact_sheet_sha256": sha(CONTACT)}, ensure_ascii=False))
