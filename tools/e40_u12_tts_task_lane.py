#!/usr/bin/env python3
"""CAS-register and close the E40 U12 exactly-one TTS production lane."""
...
STORE_DIR = Path("/Users/rogerwu/.local/share/backlotos/share/pipeline-tools")
sys.path.insert(0, str(STORE_DIR))
from task_lane_state_store import commit_task_updates, read_scheduler_snapshot  # noqa: E402

TASK_ID = "E40-U12-DIA010-EXACTLY-ONE-TTS-EXECUTION"
...
def main() -> int:
    ...
    current = next(
        (copy.deepcopy(row) for row in snapshot.payload.get("tasks", []) if row.get("task_id") == TASK_ID),
        None,
    )
    if current is None:
        ...
        current = {
            "task_id": TASK_ID,
            "lane_id": "AUDIO_PRODUCTION",
            "state": "RUNNING",
            "zero_cost": False,
            "deliverable_type": "EXACTLY_ONE_EXACT_DIALOGUE_TTS_AND_QA",
            "priority": 100,
            "scope": ["E40", "U12", "DIA010", "TTS", "EXACTLY_ONCE"],
            "exact_predecessor_task_id": "E40-U12-DIA010-EXACT-AUDIO-SOURCE-AND-VOICE-BINDING-PREFLIGHT",
            "liveness_role": "PRODUCING",
            "observation_only": False,
            "maximum_new_submissions": 1,
            "authorization": True,
        }
    elif args.action == "register" and current.get("state") != "TERMINAL":
        ...
        current.update(
            {
                "state": "RUNNING",
                "liveness_role": "PRODUCING",
                "lease_owner": "codex-e40-u12:dia010-exactly-one-tts",
    ...
    result = commit_task_updates(
        ...
        writer_id="codex-e40-u12:dia010-exactly-one-tts",
    )
    print(result)
    return 0
