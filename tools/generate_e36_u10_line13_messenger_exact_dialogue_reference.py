#!/usr/bin/env python3
"""Generate and context-check canonical E36 line 13 for the U10 recovery lane."""
...
REPAIR_MODE = os.environ.get("E36_U10_LINE13_PROSODY_REPAIR_MODE") == "1"
OUT = ROOT / (
    "working_assets/e36_dialogue_audio_refs_20260730/u10_line13_prosody_r2"
    if REPAIR_MODE else "working_assets/e36_dialogue_audio_refs_20260730/u10_line13"
)
QA_DIR = ROOT / "qa/e36_agentcut_20260730/u10_line13_video_runtime"
RECEIPT_DIR = ROOT / "workflow/tasks"
SUMMARY = QA_DIR / (
    "E36_U10_LINE13_MESSENGER_PROSODY_R2_SOURCE_GENERATION_SUMMARY_V1.json"
    if REPAIR_MODE else "E36_U10_LINE13_MESSENGER_EXACT_DIALOGUE_SOURCE_GENERATION_SUMMARY_V1.json"
)
...
LINE = {
    "line_number": 13,
    "dia_id": "E36-U10-L13-D01",
    "text": "小的自己也纳闷！那信封里头一个字都没有，空的！",
    "synthesis_text": "小的自己，也纳闷。那信封里头，一个字都没有。空的！" if REPAIR_MODE else "小的自己也纳闷！那信封里头一个字都没有，空的！",
    "speed": "0.96" if REPAIR_MODE else "1.06",
    "emotion": "洛城普通递信人被缚初审，分四口气慢慢辩解，在自己、纳闷、信封、一个字、空的处清楚咬字，困惑带泪、气息发紧，自然中文普通话，不喊麦，不做现代播音腔" if REPAIR_MODE else "洛城普通递信人被缚初审，困惑又害怕，近乎带泪地辩解，信封和空的咬字清楚，自然中文普通话，气息发紧，不喊麦，不做现代播音腔",
    "duration_min": 3.0,
    "duration_max": 7.5,
}
...
def canonical_gate() -> dict:
    ...
    checks = {
        ...
        "line13_exact": contract[13]["text"] == LINE["text"],
        "line13_currently_unproven": not contract[13]["covered_by_bound_accepted_transcripts"],
    }
    return {
...
def generate(model: WhisperModel) -> dict:
    row = LINE
    ...
    command = [
        str(AGENTCUT), "speech-generate", row["synthesis_text"], "--voice-id", VOICE_ID,
        "--emotion", row["emotion"], "--speed", row["speed"], "--output-dir", str(OUT),
    ...
    if completed.returncode or payload.get("status") != "completed":
        result = {
            "schema": "qingshan.exact_dialogue_audio_generation.v1", "status": "FAIL",
            "source_cl2x": SOURCE_CL2X, "source_mailbox_sha256": SOURCE_MAILBOX_SHA,
            "started_at_utc": started, "finished_at_utc": datetime.now(timezone.utc).isoformat(),
            "spoken_text": row["text"], "synthesis_text": row["synthesis_text"],
            "response": payload, "stderr": completed.stderr[-4000:],
    ...
    qa = {
        "schema": "qingshan.dialogue_audio_reference_qa.v1", "episode": "E36", "unit_id": "U10-L13",
        "dia_id": stem, "canonical_line_number": row["line_number"], "source_cl2x": SOURCE_CL2X,
        "source_mailbox_sha256": SOURCE_MAILBOX_SHA, "status": status, "expected_text": row["text"],
        "synthesis_text": row["synthesis_text"], "asr_transcript": transcript, "asr_similarity": round(similarity, 4),
        "duration_seconds": duration, "wav_path": rel(wav), "wav_sha256": sha(wav),
        "failures": [] if status == "PASS" else ["ASR_RECALL_OR_DURATION_FAIL"],
    }
    qa_path.write_text(json.dumps(qa, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    result = {
        "schema": "qingshan.exact_dialogue_audio_generation.v1", "status": status,
        "source_cl2x": SOURCE_CL2X, "source_mailbox_sha256": SOURCE_MAILBOX_SHA,
        "started_at_utc": started, "finished_at_utc": datetime.now(timezone.utc).isoformat(),
        "task_id": payload["taskId"], "voice_id": VOICE_ID, "spoken_text": row["text"],
        "synthesis_text": row["synthesis_text"], "mp3_path": rel(mp3), "mp3_sha256": sha(mp3),
        "wav_path": rel(wav), "wav_sha256": sha(wav), "qa_receipt": rel(qa_path),
        "qa_receipt_sha256": sha(qa_path), "credit": credit,
    }
    receipt_path.write_text(json.dumps(result, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    return result
...
def main() -> int:
    ...
    charged = int(result["credit"].get("charged_credits") or 0)
    status = "PASS_CONTEXTUAL_SOURCE_READY_FOR_ROBUST_ASR" if result["status"] == "PASS" and result["credit"]["status"] == "KNOWN_EXACT_TASK_STATEMENT" else "FAIL_PRESERVED"
    summary = {
        "schema": "qingshan.e36.u10_line13.messenger_exact_dialogue_source_generation.v1",
        "episode": "E36", "unit_id": "U10-L13", "source_cl2x": SOURCE_CL2X,
        "source_mailbox_sha256": SOURCE_MAILBOX_SHA, "status": status, "canonical_gate": gate,
        "result": result, "exact_new_credits": charged, "episode_total_after_generation": (7849 if REPAIR_MODE else 7847) + charged,
        "episode_cap": 10000, "video_submission_allowed": False, "changed_input_mode": "PROSODY_SPLIT_R2" if REPAIR_MODE else "ORIGINAL_SOURCE_V1",
        "blocked_by": "ROBUST_OPENCC_NORMALIZED_UNCONDITIONED_ASR_REQUIRED_BEFORE_VIDEO",
        "next_action": "Run independent unconditioned base+small x beam1/5/8 x VAD off/on QA. Build no video package unless exact12/12.",
    }
