#!/usr/bin/env python3
"""V2 fail-closed validator for E40/U12 snapshot-before-upgrade requests.

This tool is read-only with respect to the target validator. It proves that exact
prior bytes already exist in an independent, single-link content-addressed
archive before an upgrade may be considered; it never grants authority.
"""
...
POLICY_SCHEMA = "qingshan.e40.u12.v30.immutable_pre_upgrade_snapshot_policy.v2"
REQUEST_SCHEMA = "qingshan.e40.u12.v22.validator_upgrade_snapshot_request.v1"
GATE_SCHEMA = "qingshan.e40.u12.v30.immutable_snapshot_upgrade_gate.v2"
...
def validate(policy: dict[str, Any], request: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        archive_stat = archive.stat() if archive_exists and not archive_symlink else None
    except (ValueError, OSError):
    ...
    target_identity = (
        {"st_dev": target_stat.st_dev, "st_ino": target_stat.st_ino, "st_nlink": target_stat.st_nlink}
        if target_stat
        else None
    )
    archive_identity = (
        {"st_dev": archive_stat.st_dev, "st_ino": archive_stat.st_ino, "st_nlink": archive_stat.st_nlink}
        if archive_stat
        else None
    )
    inode_independent = bool(
        target_stat
        and archive_stat
        and (target_stat.st_dev, target_stat.st_ino) != (archive_stat.st_dev, archive_stat.st_ino)
    )
    archive_link_count_one = bool(archive_stat and archive_stat.st_nlink == 1)
    check(
        checks,
        "PRIOR_ARCHIVE_DEVICE_INODE_DIFFERS_FROM_TARGET",
        inode_independent,
        {"target": target_identity, "archive": archive_identity},
    )
    check(
        checks,
        "PRIOR_ARCHIVE_LINK_COUNT_ONE",
        archive_link_count_one,
        archive_identity,
    )
