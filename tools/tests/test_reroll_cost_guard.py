...
class RerollCostGuardTests(unittest.TestCase):
    ...
    def test_refunded_provider_failure_does_not_consume_reroll_budget(self):
        ledger = {
            "events": [
                {
                    "shot_id": "S1",
        ...
        result = evaluate(
            ...
            failure_class="PROVIDER_TRANSIENT_FAILURE",
            provider_recovery_action="reduced to one line and verified healthy route",
            total_paid_tasks=1,
        )
        self.assertEqual(result["status"], "PASS_PROVIDER_RECOVERY_RETRY_ALLOWED")
    ...
    def test_provider_retry_is_blocked_until_recovery_action_exists(self):
        result = evaluate(
            POLICY,
            {"events": []},
            shot_id="S1",
        ...
        self.assertIn("PROVIDER_RECOVERY_ACTION_REQUIRED", result["failures"])
