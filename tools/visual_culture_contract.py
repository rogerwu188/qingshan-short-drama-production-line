...
DEFAULT_CONTRACT: dict[str, Any] = {
    ...
    "decision_basis": "剧本世界、时代、地域与项目视觉圣经",
    "source_ref": "WRITER_GENERATION_CONTRACT",
...
def _episode_number(payload: dict[str, Any]) -> int | None:
    match = re.match(r"E(\d+)", str(payload.get("episode") or payload.get("unit_id") or "").upper())
...
def validate_visual_culture_contract(payload: dict[str, Any], *, prompt_text: str | None = None) -> dict[str, Any]:
    ...
    if isinstance(contract, dict):
        ...
        for field in ("decision_basis", "source_ref"):
