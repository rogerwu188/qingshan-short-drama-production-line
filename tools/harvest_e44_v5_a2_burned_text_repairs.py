#!/usr/bin/env python3
"""Query/download the three bound E44 A2 repairs; never submit."""
...
SUBMISSION = ROOT / "qa/e44_v5_a2_burned_text_repairs/E44_V5_A2_BURNED_TEXT_SUBMISSION_V1.json"
ASSET_DIR = ROOT / "working_assets/e44_v5_video_units_a2_burned_text"
RAW_DIR = ROOT / "qa/e44_v5_a2_burned_text_repairs/raw_status"
OUT = ROOT / "qa/e44_v5_a2_burned_text_repairs/E44_V5_A2_BURNED_TEXT_HARVEST_LATEST.json"
...
def one(task: dict, api_key: str) -> dict:
    ...
    unit_id = task_key.split("-VIDEO-A2")[0]
    ...
    row = {"task_key": task_key, "unit_id": unit_id, "task_id": task_id, "status": status, "raw_response": str(raw.relative_to(ROOT))}
...
def main() -> int:
    ...
    if submission.get("status") != "PASS" or submission.get("submitted") != 3 or len(tasks) != 3 or len({row.get("task_id") for row in tasks}) != 3:
        raise ValueError("E44 A2 submission is not the expected exact three-task batch")
    with ThreadPoolExecutor(max_workers=3) as pool:
        rows = [future.result() for future in as_completed([pool.submit(one, task, api_key) for task in tasks])]
    ...
    payload = {
        "schema": "qingshan.e44.v5.a2_burned_text_harvest.v1",
        "episode": "E44",
        "observed_at": now(),
    ...
    print(json.dumps({"status_counts": counts, "all_terminal": payload["all_terminal"], "all_completed": payload["all_completed"], "out": str(OUT.relative_to(ROOT))}, ensure_ascii=False))
    return 0
