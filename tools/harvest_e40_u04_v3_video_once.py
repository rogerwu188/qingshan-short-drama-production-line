#!/usr/bin/env python3
"""Query bound E40 U04 V3 task once and download once if completed."""
...
RAW = ROOT / f"working_assets/e40_production_20260814/raw_u04_v3_fast720/{TASK_KEY}_{TASK_ID}.json"
OUTPUT = ROOT / f"working_assets/e40_production_20260814/u04_v3_fast720/{TASK_KEY}_{TASK_ID}.mp4"
REPORT = ROOT / "workflow/tasks/E40_U04_V3_FAST720_HARVEST_20260814.json"
...
def download_once(url, output):
    if output.exists():
        raise RuntimeError("output already exists; repeat download forbidden")
    output.parent.mkdir(parents=True, exist_ok=True)
    partial = output.with_suffix(".mp4.part")
    if partial.exists():
        raise RuntimeError("partial output exists; manual classification required")
    request = urllib.request.Request(url, headers={"User-Agent": "qingshan-e40-u04-v3-harvester/1.0"})
    try:
...
def main():
    if REPORT.exists() or RAW.exists() or OUTPUT.exists():
        raise SystemExit("harvest artifact already exists; repeat query/download forbidden")
    ...
    report = {
        "schema": "qingshan.e40.u04.v3.fast720_video_harvest.v1",
        "recorded_at": queried_at,
        "task_key": TASK_KEY,
        "task_id": TASK_ID,
        "transaction": portable(TX),
        "transaction_sha256": sha(TX),
        "query_count": 1,
        "download_count": 0,
        "remote_status": status,
        "raw_response": portable(RAW),
        ...
        report.update(
            {
                ...
                "next_action": "Run mandatory technical, exact-frame, cadence, OCR and original-resolution human QA.",
            }
        )
        ...
        report.update(
            {
                ...
                "failure_reason": data.get("err_msg") or status,
        ...
        report.update(
            {
                ...
                "next_action": "Keep task-local REMOTE_WAIT and query this exact task_id once at the next scheduled wakeup.",
            }
        )
    write(REPORT, report)
