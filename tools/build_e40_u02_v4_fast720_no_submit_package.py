#!/usr/bin/env python3
"""Build and fail-closed validate E40 U02 V4 Fast720 no-submit package."""
...
BASE=ROOT/'workflow/claude_writer_agent/production/e40_claude_writer_v3_140d4b7b_20260808/u02_v4_fast720_exact_start_frame_v1'
PROMPT=BASE/'E40_U02_V4_FAST720_SILENT_VISUAL_PROMPT_V1.txt'
MANIFEST=BASE/'E40_U02_V4_FAST720_NO_SUBMIT_MANIFEST_V1.json'
GATE=ROOT/'qa/e40_preproduction_20260814/u02_v4_fast720_no_submit_package_qa_v1/E40_U02_V4_FAST720_STATIC_GATE_V1.json'
...
def portable(p): return str(Path(p).relative_to(ROOT))
def write(p,d):
 p=Path(p); p.parent.mkdir(parents=True,exist_ok=True); fd,n=tempfile.mkstemp(prefix='.'+p.name+'.',dir=p.parent)
 try:
...
def raw_rgb_sha(p):
 with Image.open(p) as im:
  rgb=im.convert('RGB'); w,h=rgb.size; payload=struct.pack('>QQ',w,h)+rgb.tobytes()
 return hashlib.sha256(payload).hexdigest(),w,h
...
def main():
 checks={
  ...
  'human_qa_sha_and_pass':sha(FRAME_QA)=='a397247256c2eec8cb85ea0452d0494f202acc81cb3e18e2a8518dfeccea4426' and json.loads(FRAME_QA.read_text()).get('status')=='PASS_ADMITTED_EXACT_START_FRAME_ONLY',
  'prompt_fast_only':('seedance-2.0-fast' in PROMPT.read_text() and 'seedance-2.0-pro' not in PROMPT.read_text().lower() and 'seedance-2.0-mini' not in PROMPT.read_text().lower()),
  'prompt_720p':('720p' in PROMPT.read_text()),
  'prompt_silent_visual':('无声视觉源' in PROMPT.read_text() and '不得生成口型' in PROMPT.read_text()),
  'prompt_low_hem_lock':('画高不超过2%' in PROMPT.read_text() and '帘脚仍贴底' in PROMPT.read_text()),
  'prompt_no_literal_dialogue':('阿栓，在本宫手上' not in PROMPT.read_text() and '拿他，换景朝一个接头人' not in PROMPT.read_text()),
 }
 if not all(checks.values()): raise SystemExit('static prebuild failed '+json.dumps(checks,ensure_ascii=False))
 prompt_sha=sha(PROMPT); raw,w,h=raw_rgb_sha(FRAME)
 gate={'schema':'qingshan.e40.u02.v4.fast720_static_gate.v1','recorded_at':'2026-08-14T06:16:00Z','status':'PASS','checks':checks,
  'model':'seedance-2.0-fast','resolution':'720p','duration_seconds':6,'exact_start_frame_sha256':EXPECTED_FRAME,'prompt_sha256':prompt_sha,
  'provider_posts':0,'transactions':0,'credits':0,'maximum_new_submissions':0}
 write(GATE,gate)
 task={'task_key':'E40-U02-V4-FAST720-LOW-HEM-EXACT-FIRST-FRAME-SILENT-V1','unit_id':'U02','scene_id':'13-1','kind':'hidden_speaker_silent_visual_closeup',
  ...
  'aspect_ratio':'9:16','duration_seconds':6,'generating_count':1,'prompt_file':portable(PROMPT),'prompt_sha256':prompt_sha,
  ...
  'frame0_authority_contract':{'source_sha256':EXPECTED_FRAME,'pre_encode_raw_rgb_sha256_required':True,'raw_rgb_sha256':raw,'width':w,'height':h,'semantic_start_frame_human_score':88},
  ...
  'performance_tempo_contract':{'playback_speed':'REAL_TIME_1X','first_visible_displacement_by_seconds':0.45,'primary_action_complete_by_seconds':5.2,'maximum_atomic_window_seconds':1.2,'maximum_action_gap_seconds':0.25,'slow_motion_interpolation_post_speedup_forbidden':True,'atomic_action_windows':[
   {'start_seconds':0.0,'end_seconds':0.45,'action':'半合扇骨继续收拢且拇指压紧','state_change':'扇骨间距缩小'},
   {'start_seconds':0.45,'end_seconds':1.4,'action':'扇骨短弧靠近并轻叩一次','state_change':'约三分之二收拢'},
   {'start_seconds':1.4,'end_seconds':2.35,'action':'手腕下降半掌且袖口跟随','state_change':'持扇点下移'},
   {'start_seconds':2.35,'end_seconds':3.3,'action':'手腕内旋且扇骨投影压窄','state_change':'扇面角度改变'},
   {'start_seconds':3.3,'end_seconds':4.25,'action':'素纱中段微鼓且帘脚贴底','state_change':'帘中段位移'},
   {'start_seconds':4.25,'end_seconds':5.2,'action':'拇指压住未完全闭合终态','state_change':'扇骨稳定未闭死'},
   {'start_seconds':5.2,'end_seconds':6.0,'action':'轻叩余势和纤维非循环微变','state_change':'在活态中切出'}]},
 ...
 manifest={'schema':'qingshan.e40.u02.v4.fast720_no_submit_manifest.v1','episode':'E40','recorded_at':'2026-08-14T06:16:00Z','status':'READY_LOCAL_PRECHECK_ONLY_NO_PAID_AUTHORIZATION','provider':'giggle',
  ...
  'next_action':'Run installed submitter only with --precheck-only, then separately certify paid readiness before any provider POST.'}
 write(MANIFEST,manifest)
 print(json.dumps({'status':'PASS','prompt_sha256':prompt_sha,'gate_sha256':sha(GATE),'manifest_sha256':sha(MANIFEST),'raw_rgb_sha256':raw}))
