#!/usr/bin/env python3
"""Generate and machine-QA release-clear local U10 DIA-009 audio candidates."""
...
TEXT = "……你倒看得清楚。"
TARGET_DURATION = 2.0
OUT = ROOT / "working_assets/e40_production_20260814/u10_parallel_kokoro_exact_audio_candidates_v1"
PRE_QA = ROOT / "qa/e40_preproduction_20260814/u10_parallel_kokoro_exact_audio_candidates_v1"
PROD_QA = ROOT / "qa/e40_production_20260814/u10_parallel_kokoro_exact_audio_candidates_v1"
RECEIPT = PRE_QA / "E40_U10_PARALLEL_KOKORO_CANDIDATE_GENERATION_RECEIPT_V1.json"
RIGHTS = PRE_QA / "E40_U10_PARALLEL_KOKORO_COMMERCIAL_RIGHTS_EVIDENCE_V1.json"
QA = PROD_QA / "E40_U10_PARALLEL_KOKORO_EXACT_AUDIO_MACHINE_QA_V1.json"
V2_OUT = ROOT / "working_assets/e40_production_20260814/u10_parallel_kokoro_exact_audio_candidates_v2"
V2_QA = ROOT / "qa/e40_production_20260814/u10_parallel_kokoro_exact_audio_candidates_v2/E40_U10_PARALLEL_KOKORO_EXACT_AUDIO_MACHINE_QA_V2.json"
FAILURE_MEMORY = PROD_QA / "E40_U10_PARALLEL_KOKORO_V1_FAILURE_MEMORY_AND_V2_REPAIR_CONTRACT.json"
...
def rewrite_generation_receipts() -> None:
    ...
    for item in payload["outputs"]:
        for key in ("source_path", "normalized_path"):
            old = ROOT / item[key]
            new = old.with_name(old.name.replace("E40-DIA006_", "E40-DIA009_"))
            if new.exists():
    ...
    payload.update(
        {
            "schema": "qingshan.e40.u10.parallel.kokoro_candidate_generation.v1",
            "episode": "E40",
            "unit_id": "U10",
            "scene_id": "13-2",
            "dialogue_id": "E40-DIA-009",
            "speaker": "云妃",
            "expected_text": TEXT,
    ...
    rights.update(
        {
            "schema": "qingshan.e40.u10.parallel.kokoro_commercial_rights_evidence.v1",
            "episode": "E40",
            "unit_id": "U10",
            "dialogue_id": "E40-DIA-009",
            "speaker": "云妃",
            "voice_type": "BUILT_IN_MODEL_CHINESE_FEMALE_SPEAKER_NOT_USER_CLONE",
            "continuity_basis": "Preserve admitted E40 U02-U03 Yunfei voice identity with built-in zf_001.",
        }
    )
...
def generate() -> int:
    ...
    print(json.dumps({"status": "PASS_ZERO_CREDIT_U10_CANDIDATES_GENERATED_QA_PENDING", "candidates": 3}, ensure_ascii=False))
    return 0
...
def failures(similarity: float, audio: dict) -> list[str]:
    ...
    if not 1.3 <= audio["duration_seconds"] <= 3.2:
        failed.append("DURATION_OUTSIDE_1P3_TO_3P2")
...
def qa() -> int:
    ...
    payload = {
        "schema": "qingshan.e40.u10.parallel.kokoro_exact_audio_machine_qa.v1",
        "episode": "E40", "unit_id": "U10", "scene_id": "13-2", "dialogue_id": "E40-DIA-009", "speaker": "云妃",
        ...
        "selection_rule": "exact normalized ASR and audio gates; preserve admitted zf_001 identity; duration nearest canonical 2.0s",
...
def repair_qa() -> int:
    ...
    selected = passes[0] if passes else None
    payload = {
        "schema": "qingshan.e40.u10.parallel.kokoro_exact_audio_machine_qa.v2",
        "episode": "E40", "unit_id": "U10", "scene_id": "13-2", "dialogue_id": "E40-DIA-009", "speaker": "云妃",
