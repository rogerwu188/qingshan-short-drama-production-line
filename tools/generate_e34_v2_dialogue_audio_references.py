#!/usr/bin/env python3
"""Generate and QA E34 v2 exact dialogue references from the current voice registry."""
...
EPISODE = os.environ.get("DIALOGUE_AUDIO_EPISODE", "E34")
...
INVENTORY = ROOT / os.environ.get("DIALOGUE_AUDIO_INVENTORY", "workflow/claude_writer_agent/production/e34_claude_writer_v2_400ff6d2_20260723/E34_SCRIPT_BEAT_DIALOGUE_INVENTORY_V2.json")
OUT_DIR = ROOT / os.environ.get("DIALOGUE_AUDIO_OUT_DIR", "working_assets/e34_dialogue_audio_refs_v2_20260723")
QA_DIR = ROOT / os.environ.get("DIALOGUE_AUDIO_QA_DIR", "qa/e34_dialogue_audio_refs_v2_20260723")
MANIFEST = OUT_DIR / os.environ.get("DIALOGUE_AUDIO_MANIFEST_NAME", "E34_DIALOGUE_AUDIO_REFERENCE_MANIFEST_V2.json")
RECEIPT_DIR = ROOT / os.environ.get("DIALOGUE_AUDIO_RECEIPT_DIR", "workflow/tasks/e34_dialogue_audio_refs_v2_20260723")
EXPECTED_LINE_COUNT = int(os.environ.get("DIALOGUE_AUDIO_EXPECTED_LINE_COUNT", "43"))
ALLOWED_SPEAKERS = set(os.environ.get("DIALOGUE_AUDIO_ALLOWED_SPEAKERS", "chenji,jiaotu,yunyang,passerby_hushed,yanjing").split(","))
...
def task_credit(task_id: str) -> dict:
    ...
    for _ in range(7):
        ...
        time.sleep(2)
    return {"status": "UNKNOWN_NOT_ESTIMATED", "charged_credits": None, "task_id": task_id, "statement_rows": [], "query_errors": errors}


def write_line_receipt(row: dict) -> None:
    RECEIPT_DIR.mkdir(parents=True, exist_ok=True)
    (RECEIPT_DIR / f"{row['dia_id']}.json").write_text(
        json.dumps(row, ensure_ascii=False, indent=2) + "\n", encoding="utf-8"
    )


def recover_existing(rows: list[dict], specs: dict[str, dict], canonical: dict[str, dict]) -> list[dict]:
    """Bind already-downloaded MP3s back to exact task statements without regenerating."""
    response = _get("/api/v1/payment/credit-statements", {"credit_type": "Pay", "page": 1, "page_size": 100})
    statements = [
        row for row in ((response.get("data") or {}).get("list") or [])
        if row.get("event_description") == "SingleGenerateAudio"
        and row.get("event_type") == "Pay"
        and row.get("project_id")
    ]

    def remote_digest(statement: dict) -> tuple[str, str | None, str | None]:
        task_id = str(statement["project_id"])
        try:
            task = _get("/api/v1/generation/task/query", {"task_id": task_id})
            urls = ((task.get("data") or {}).get("urls") or [])
            if not urls:
                return task_id, None, "completed task has no URL"
            with urllib.request.urlopen(str(urls[0]).replace("~", "%7E"), timeout=120) as stream:
                return task_id, hashlib.sha256(stream.read()).hexdigest(), None
        except BaseException as exc:
            return task_id, None, f"{type(exc).__name__}: {exc}"

    remote_by_sha: dict[str, tuple[str, dict]] = {}
    query_errors = []
    with ThreadPoolExecutor(max_workers=min(8, len(statements))) as pool:
        futures = {pool.submit(remote_digest, statement): statement for statement in statements}
        for future in as_completed(futures):
            statement = futures[future]
            task_id, digest_value, error = future.result()
            if digest_value:
                remote_by_sha[digest_value] = (task_id, statement)
            elif error:
                query_errors.append({"task_id": task_id, "error": error})

    recovered = []
    for row in rows:
        role = row["speaker"]
        spec = specs[role]
        mp3 = OUT_DIR / "raw" / f"{row['dialogue_id']}.mp3"
        match = remote_by_sha.get(sha(mp3))
        if match:
            task_id, statement = match
            credit_value = abs(Decimal(str(statement["credit"])))
            credit = {
                "status": "KNOWN_EXACT_TASK_STATEMENT_RECOVERED_BY_AUDIO_SHA256",
                "charged_credits": int(credit_value) if credit_value == credit_value.to_integral() else str(credit_value),
                "task_id": task_id,
        ...
        item = {
            ...
            "voice_reference_asset_id": canonical[role]["remote_asset_id"],
            "recovery": "LOCAL_MP3_SHA256_MATCHED_TO_REMOTE_COMPLETED_AUDIO_TASK" if match else "LOCAL_MP3_PRESERVED_NO_REGENERATION",
        }
        write_line_receipt(item)
...
def main() -> int:
    ...
    if len(lines) != EXPECTED_LINE_COUNT:
        raise SystemExit(f"{EPISODE} dialogue inventory must contain {EXPECTED_LINE_COUNT} lines")
    ...
    if any(row["speaker"] not in ALLOWED_SPEAKERS for row in lines):
        raise SystemExit(f"{EPISODE} dialogue contains an unregistered speaker")
    missing_voice_roles = sorted({row["speaker"] for row in generated_lines} - specs.keys())
    missing_registry_roles = sorted({row["speaker"] for row in generated_lines} - canonical.keys())
    if missing_voice_roles or missing_registry_roles:
        raise SystemExit(
            f"E34 v2 voice binding incomplete: policy={missing_voice_roles}, registry={missing_registry_roles}"
        )

    existing_lines = [row for row in generated_lines if (OUT_DIR / "raw" / f"{row['dialogue_id']}.mp3").is_file()]
    missing_lines = [row for row in generated_lines if row not in existing_lines]
    generated: list[dict] = recover_existing(existing_lines, specs, canonical) if existing_lines else []
    with ThreadPoolExecutor(max_workers=8) as pool:
        futures = [pool.submit(generate, row, specs, canonical) for row in missing_lines]
        for future in as_completed(futures):
