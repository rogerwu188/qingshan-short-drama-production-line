#!/usr/bin/env python3
"""Build an exact-SHA 24-shot visual continuity review for E27 AgentCut v15."""
...
TIMELINE = ROOT / "qa/e27_agentcut_v15_writer_agent_v040_native_text_r2_20260720/E27_FINAL_TIMELINE_SHOTS.json"
OUT = ROOT / "qa/e27_agentcut_v15_writer_agent_v040_native_text_r2_visual_review_20260720"
...
def main() -> int:
    ...
    for index, shot in enumerate(timeline["shots"], 1):
        ...
        subprocess.run(
            [
                ...
                "-frames:v", "1", "-vf", "scale=180:320:force_original_aspect_ratio=decrease,pad=180:320:(ow-iw)/2:(oh-ih)/2:black",
                str(frame),
            ],
            check=True,
        )
        frames.append(frame)
    sheet = OUT / "E27_AGENTCUT_V15_24_SHOT_CONTACT_SHEET.png"
    command = ["ffmpeg", "-hide_banner", "-loglevel", "error", "-y"]
    for frame in frames:
        command.extend(["-i", str(frame)])
    inputs = "".join(f"[{index}:v]" for index in range(len(frames)))
    command.extend(["-filter_complex", f"{inputs}xstack=inputs=24:layout=0_0|180_0|360_0|540_0|0_320|180_320|360_320|540_320|0_640|180_640|360_640|540_640|0_960|180_960|360_960|540_960|0_1280|180_1280|360_1280|540_1280|0_1600|180_1600|360_1600|540_1600[out]", "-map", "[out]", str(sheet)])
    subprocess.run(command, check=True)
    request = OUT / "E27_AGENTCUT_V15_24_SHOT_VISUAL_REVIEW_REQUEST.json"
    request.write_text(
        json.dumps(
            {
                "items": [
                    {
                        ...
                        "importance": "critical",
                        "pass_score": 4.5,
                        "clip_id": "E27-AGENTCUT-V15-24-SHOT-CONTACT-SHEET",
                        "metadata": {
                            "episode": "E27",
    ...
    inventory = OUT / "E27_AGENTCUT_V15_24_SHOT_VISUAL_REVIEW_INVENTORY.json"
