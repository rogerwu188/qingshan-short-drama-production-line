...
RUN_DIR = Path(os.environ.get("E11_RUN_DIR", BASE / "working_assets/e11_api_20260710"))
REPAIR_RUN_DIR = Path(os.environ.get("E11_REPAIR_DIR", BASE / "working_assets/e11_api_repair_20260710"))
REPAIR2_RUN_DIR = Path(os.environ.get("E11_REPAIR2_DIR", BASE / "working_assets/e11_api_repair2_20260710"))
OUT_DIR = Path(os.environ.get("E11_OUT_DIR", BASE / "exports/e11/final_package_20260710"))
QA_DIR = Path(os.environ.get("E11_QA_DIR", BASE / "qa/e11_final_package_20260710"))
SEG_DIR = OUT_DIR / "segments"
SUBTITLE_TEXT_DIR = OUT_DIR / "subtitle_texts_clean_20260710"
LOGO = BASE / "libraries/brand/nalu_motion_cat_logo_v1.png"

RAW = OUT_DIR / "qingshan_E11_raw_normalized_20260710.mp4"
TAILED = OUT_DIR / "qingshan_E11_tailed_nalu_20260710.mp4"
FINAL = OUT_DIR / "qingshan_E11_final_subtitled_nalu_20260710.mp4"
CONTACT = QA_DIR / "qingshan_E11_final_subtitled_contact_20260710.jpg"
MIDPOINT_CONTACT = QA_DIR / "qingshan_E11_final_all_shots_midpoint_20260710.jpg"
SOURCE_MANIFEST = OUT_DIR / "e11_final_source_manifest_20260710.json"
LOUDNESS_REPORT = QA_DIR / "qingshan_E11_final_loudness_20260710.txt"
...
def source_for(shot: int) -> tuple[str, Path]:
    if shot in REPAIRS:
        if shot in REPAIRS2:
            path = REPAIR2_RUN_DIR / f"videos/shot_{shot:02d}/result_01.mp4"
            source = "repair2"
        else:
            path = REPAIR_RUN_DIR / f"videos/shot_{shot:02d}/result_01.mp4"
            source = "repair"
    else:
        path = RUN_DIR / f"videos/shot_{shot:02d}/result_01.mp4"
        source = "api"
    if not path.exists():
        raise SystemExit(f"Missing E11 source clip: {path}")
    return source, path
...
def make_tail_card(dst: Path) -> None:
    ...
    filter_complex = (
        ...
        "drawtext=fontfile=/System/Library/Fonts/STHeiti\\ Medium.ttc:"
        "text='第12集继续':fontcolor=white:fontsize=38:x=(w-text_w)/2:y=520,"
...
def write_subtitle_textfiles() -> None:
    ...
    for idx, text in SUBTITLES.items():
        ...
        for line_no, line in enumerate(text.split("\\N"), 1):
            clean = re.sub(r"^[^：:]{1,8}[：:]", "", line).strip()
            (SUBTITLE_TEXT_DIR / f"shot_{idx:02d}_line_{line_no:02d}.txt").write_text(clean, encoding="utf-8")
...
def main() -> int:
    ...
    for shot in range(1, 21):
        source, src = source_for(shot)
        dst = SEG_DIR / f"seg_{shot:03d}_shot_{shot:02d}.mp4"
        normalize(src, dst)
        shot_durations.append(duration(dst))
        segments.append(dst)
        manifest.append({"shot": f"{shot:02d}", "source": source, "path": str(src), "normalized": str(dst)})
    ...
    SOURCE_MANIFEST.write_text(json.dumps({
        ...
        "raw": str(RAW),
        "tailed": str(TAILED),
        "rule": "No opening title card; first frame remains the crisis hook. Subtitles are clean dialogue only, with speaker prefixes stripped.",
        "repairs_used": sorted(REPAIRS),
        "shots": manifest,
        "shot_durations": shot_durations,
        "duration_seconds": total,
    }, ensure_ascii=False, indent=2), encoding="utf-8")

    contact_times = [2, 6, 12, 20, 30, 40, 50, 60, 72, 84, 96, 108, 119, 125]
    contact_times = [t for t in contact_times if t < total - 1]
    extract_contact(FINAL, CONTACT, contact_times, "final_subtitle_frames", "4x4")
