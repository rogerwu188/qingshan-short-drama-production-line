#!/usr/bin/env python3
"""Canonical and substitution-negative matrix for V56."""
from __future__ import annotations
import argparse,hashlib,json,subprocess,sys
from datetime import datetime,timezone
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1];INVOKER=ROOT/'tools/run_e40_u12_v56_pinned_safety_ledger.py'
...
def main():
 ...
 ok=run.returncode==0 and gate.get('status')=='PASS_V46_TO_V54_NINE_CLOSEOUTS_EXACT_AUTHORITY_ADMISSION_CLOSED' and gate.get('entry_pass_count')==9 and gate.get('new_evidence_synthesized') is False and gate.get('authority_keys_admitted')==0 and gate.get('production_assets_admitted')==0 and gate.get('authorization') is False
 cases.append({'case_id':'CANONICAL_PINNED_NINE_CLOSEOUT_LEDGER','returncode':run.returncode,'output_created':canonical.exists(),'output_sha256':sha(canonical) if canonical.exists() else None,'passed':ok})
 for case_id,arg in [('SPEC_SUBSTITUTION_REJECTED','--spec'),('LEDGER_TOOL_SUBSTITUTION_REJECTED','--ledger-tool'),('CLOSEOUT_SET_SUBSTITUTION_REJECTED','--closeout-set')]:
 ...
 report={'schema':'qingshan.e40.u12.v56.pinned_safety_ledger_regression.v1','recorded_at':datetime.now(timezone.utc).isoformat().replace('+00:00','Z'),'status':'PASS_PINNED_NINE_CLOSEOUT_LEDGER_AND_3_SUBSTITUTIONS_REJECTED_NO_ADMISSION' if passed else 'FAIL_CLOSED_PINNED_SAFETY_LEDGER_REGRESSION_REQUIRES_REVIEW','invoker_sha256':sha(INVOKER),'cases':cases,'case_count':len(cases),'case_pass_count':sum(c['passed'] for c in cases),'nested_entry_pass_count':gate.get('entry_pass_count'),'new_evidence_synthesized':False,'authority_keys_admitted':0,'production_assets_admitted':0,'real_evidence_validation_authorized':False,'positive_admission_test_authorized':False,'authorization':False,'maximum_new_submissions':0,'side_effects':{'provider_calls':0,'transactions':0,'credits':0,'generation_actions':0,'renders':0,'agentcut_actions':0,'assembly_actions':0,'release_actions':0,'browser_started':False,'platform_state_changed':False,'work_queue_changed':False,'e38_state_changed':False,'e39_state_changed':False}}
 out.parent.mkdir(parents=True,exist_ok=True);out.write_text(json.dumps(report,ensure_ascii=False,indent=2)+'\n');print(json.dumps({'status':report['status'],'cases':f"{report['case_pass_count']}/{report['case_count']}",'nested':report['nested_entry_pass_count']}));return 0 if passed else 1
