#!/usr/bin/env python3
"""Generate and verify the exact U18A Yunyang dialogue reference."""
...
OUT = ROOT / "working_assets/e36_dialogue_audio_refs_20260730/u18a"
QA = ROOT / "qa/e36_v2_stills_repair_20260729/u18_video_runtime/E36_U18A_EXACT_DIALOGUE_AUDIO_QA_V1.json"
RECEIPT = ROOT / "workflow/tasks/E36_U18A_EXACT_DIALOGUE_AUDIO_GENERATION_V1.json"
TEXT = "刘家？城东那个刘家？那户三年前就灭门了！"
...
def main() -> int:
    ...
    mp3 = OUT / "E36-U18A-D01.mp3"
    command = [
        str(AGENTCUT), "speech-generate", TEXT,
        "--voice-id", VOICE_ID,
        "--emotion", "十七岁云羊凑近物证后骤然认出刘家，震惊失声但仍是自然中文普通话，不喊麦，不做现代播音腔",
        "--speed", "1.08",
        "--output-dir", str(OUT),
        "--file-name", mp3.name,
        "--poll-interval", "2",
    ...
    status = "PASS" if similarity >= 0.80 and 2.0 <= duration <= 5.0 else "FAIL"
    credit = exact_credit(payload["taskId"])
    qa = {"schema": "qingshan.dialogue_audio_reference_qa.v1", "episode": "E36", "dia_id": "E36-U18A-D01", "status": status, "expected_text": TEXT, "asr_transcript": transcript, "asr_similarity": round(similarity, 4), "duration_seconds": duration, "wav_path": str(wav), "wav_sha256": sha(wav), "failures": [] if status == "PASS" else ["ASR_RECALL_OR_DURATION_FAIL"]}
