#!/usr/bin/env python3
"""Fresh paid readiness and exactly-one authorization for E40 U04 V3."""
...
SOURCE = ROOT / "workflow/claude_writer_agent/production/e40_claude_writer_v3_140d4b7b_20260808/u04_v3_fast720_coherent_frame_v1/E40_U04_V3_FAST720_NO_SUBMIT_MANIFEST_V1.json"
PRECHECK = ROOT / "qa/e40_preproduction_20260814/u04_v3_fast720_no_submit_package_v1/E40_U04_V3_INSTALLED_PRECHECK_ONLY_V1.json"
AUTHORIZED = ROOT / "workflow/claude_writer_agent/production/e40_claude_writer_v3_140d4b7b_20260808/u04_v3_fast720_coherent_frame_v1/E40_U04_V3_FAST720_AUTHORIZED_EXACTLY_ONCE_MANIFEST_V1.json"
OUT = ROOT / "qa/e40_preproduction_20260814/u04_v3_fast720_no_submit_package_v1/E40_U04_V3_FAST720_PAID_READINESS_V1.json"
AUTH = ROOT / "workflow/approvals/E40_U04_V3_FAST720_EXACTLY_ONCE_AUTHORIZATION_20260814.json"
...
def main() -> int:
    submitter = load_module("e40_u04_installed_submitter", SUBMITTER)
    credit = load_module("e40_u04_credit_ledger", TOOLS / "giggle_credit_statements.py")
    submitter.ROOT = ROOT
    credit.ROOT = ROOT
    ...
    report = {
        "schema": "qingshan.e40.u04.v3.fast720_paid_readiness.v1",
        "recorded_at": recorded, "status": "PASS" if all(checks.values()) else "FAIL",
        "source_manifest": str(SOURCE.relative_to(ROOT)), "source_manifest_sha256": sha256(SOURCE),
        "installed_precheck": str(PRECHECK.relative_to(ROOT)), "installed_precheck_sha256": sha256(PRECHECK),
        "task_key": task["task_key"], "task_fingerprint": fingerprint, "transport_fingerprint": transport,
        "expected_transaction_path": str(expected_transaction.relative_to(ROOT)),
        "checks": checks, "collision_scan": matches,
        ...
        "provider_posts": 0, "provider_queries": 1, "credits": 0,
    ...
    authorized["schema"] = "qingshan.e40.u04.v3.fast720_authorized_exactly_once_manifest.v1"
    ...
    authorized["submission_policy"].update({
        "precheck_only": False, "paid_submission_allowed": True, "provider_post_allowed": True,
        "durable_transaction_allowed": True, "maximum_new_submissions": 1,
    })
    authorized["blocked_by"] = "AUTHORIZED_MANIFEST_INSTALLED_PRECHECK_NOT_YET_PASSED"
    authorized["tasks"][0]["submission_authorization"] = {
        "precheck_only": False, "authorized": True, "paid_submission_allowed": True,
        "transaction_creation_allowed": True, "maximum_new_submissions": 1,
        "paid_readiness_path": str(OUT.relative_to(ROOT)), "paid_readiness_sha256": sha256(OUT),
    }
    atomic_json(AUTHORIZED, authorized)
    atomic_json(AUTH, {
        "schema": "qingshan.e40.u04.v3.fast720_exactly_once_authorization.v1",
        "recorded_at": recorded, "status": "AUTHORIZE_EXACTLY_ONE_FAST720_PROVIDER_POST_AFTER_AUTHORIZED_MANIFEST_PRECHECK_PASS",
        "authority": "Roger standing instruction: 你应该自己选择，以后不需要问我; E40 seedance-2.0-fast-only production authorization",
        "task_key": task["task_key"], "task_fingerprint": fingerprint, "transport_fingerprint": transport,
        "source_manifest_sha256": sha256(SOURCE), "paid_readiness_sha256": sha256(OUT),
        "authorized_manifest": str(AUTHORIZED.relative_to(ROOT)), "authorized_manifest_sha256": sha256(AUTHORIZED),
        "maximum_new_submissions": 1, "required_transaction_before_post": True, "task_id_binding_immediate": True,
        "unknown_response_requires_authoritative_pay_refund_classification_before_future_action": True,
        "model": "seedance-2.0-fast", "forbidden_models": ["seedance-2.0-pro", "seedance-2.0-mini", "seedance-2.0"],
    })
