#!/usr/bin/env python3
"""Query and download the 25 already-bound E44 v5 tasks; never submit."""
...
SUBMISSION = ROOT / "qa/e44_v5_video_units/E44_V5_VIDEO_SUBMISSION_V1.json"
ASSET_DIR = ROOT / "working_assets/e44_v5_video_units_a1"
RAW_DIR = ROOT / "qa/e44_v5_video_units/raw_status_a1"
OUT = ROOT / "qa/e44_v5_video_units/E44_V5_VIDEO_HARVEST_LATEST.json"
...
def one(task: dict, api_key: str) -> dict:
    ...
    row = {
        "task_key": task_key, "unit_id": unit_id, "task_id": task_id,
        "status": status, "raw_response": str(raw.relative_to(ROOT)),
    }
    ...
    if status == "completed" and urls:
        ...
        row.update({
            "video_path": str(path.relative_to(ROOT)), "video_sha256": sha(path),
...
def main() -> int:
    ...
    if submission.get("status") != "PASS" or submission.get("submitted") != 25 or submission.get("failed") != 0:
        raise ValueError("E44 v5 submission is not the expected 25/25 bound batch")
    if len(tasks) != 25 or len({row.get("task_id") for row in tasks}) != 25:
        raise ValueError("E44 v5 harvest does not bind exactly 25 unique task IDs")
    ...
    payload = {
        "schema": "qingshan.e44.video_harvest.v1", "episode": "E44", "production_version": 5,
        "observed_at": now(), "submission_ref": str(SUBMISSION.relative_to(ROOT)),
        "submission_sha256": sha(SUBMISSION), "status_counts": counts,
        "all_terminal": all(row["status"] in terminal for row in rows),
        "all_completed": all(row["status"] == "completed" for row in rows),
        "results": rows, "duplicate_post_forbidden": True,
        "post_generation_qa_scope": "TECHNICAL_AND_BASIC_PLOT_IDENTITY_ONLY",
    }
    OUT.parent.mkdir(parents=True, exist_ok=True)
    OUT.write_text(json.dumps(payload, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    print(json.dumps({
        "status_counts": counts, "all_terminal": payload["all_terminal"],
        "all_completed": payload["all_completed"], "out": str(OUT.relative_to(ROOT)),
    }, ensure_ascii=False))
    return 0
