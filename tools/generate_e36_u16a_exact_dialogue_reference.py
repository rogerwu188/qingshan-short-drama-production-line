#!/usr/bin/env python3
"""Generate and verify the exact U16A messenger dialogue reference."""
...
OUT = ROOT / "working_assets/e36_dialogue_audio_refs_20260729/u16a"
QA = ROOT / "qa/e36_v2_stills_repair_20260729/u16_video_runtime/E36_U16A_EXACT_DIALOGUE_AUDIO_QA_V1.json"
RECEIPT = ROOT / "workflow/tasks/E36_U16A_EXACT_DIALOGUE_AUDIO_GENERATION_V1.json"
TEXT = "头一回给小的银子，是从这上头支的。"
...
def main() -> int:
    ...
    mp3 = OUT / "E36-U16A-D01.mp3"
    command = [str(AGENTCUT), "speech-generate", TEXT, "--voice-id", VOICE_ID, "--emotion", "洛城普通递信人被追问后的低声交代，畏缩发紧、气息微颤但字句清楚；不可做王府传令腔或现代口播腔", "--speed", "0.98", "--output-dir", str(OUT), "--file-name", mp3.name, "--poll-interval", "2", "--timeout", "300", "--overwrite"]
    ...
    status = "PASS" if similarity >= 0.70 and 2.0 <= duration <= 15.0 else "FAIL"
    credit = exact_credit(payload["taskId"])
    qa = {"schema": "qingshan.dialogue_audio_reference_qa.v1", "episode": "E36", "dia_id": "E36-U16A-D01", "status": status, "expected_text": TEXT, "asr_transcript": transcript, "asr_similarity": round(similarity, 4), "duration_seconds": duration, "wav_path": str(wav), "wav_sha256": sha(wav), "failures": [] if status == "PASS" else ["ASR_RECALL_OR_DURATION_FAIL"]}
