#!/usr/bin/env python3
"""Poll and harvest E37 V10 exact-scene-start I2V repairs without replay."""
...
SUBMIT = ROOT / "workflow/tasks/E37_V10_I2V_START_FRAME_REPAIR_SUBMIT_V1_20260803.json"
OUT_DIR = ROOT / "working_assets/e37_video_20260803/v10_i2v_start_frame_repairs_v1"
QA_DIR = ROOT / "qa/e37_video_20260803/v10_i2v_start_frame_repairs_v1"
...
def main() -> None:
    ...
    for task in submit["tasks"]:
        ...
        if row["status"] == "completed" and urls:
            target = OUT_DIR / f"E37-L{line:03d}-V10-I2V-EXACT-START_{task_id}.mp4"
        ...
        receipt = QA_DIR / f"E37-L{line:03d}-V10-HARVEST-V1.json"
        ...
        rows.append(row)
    aggregate = {
        "schema": "qingshan.e37.v10_i2v_start_frame_harvest.v1",
        "episode": "E37",
        ...
        "counts": {
            ...
            "running": sum(r["status"] in {"running", "processing", "pending"} for r in rows),
            "failed": sum(r["status"] in {"failed", "error"} for r in rows),
        },
        "unchanged_retry": "PROHIBITED",
    }
    out = QA_DIR / "E37_V10_I2V_START_FRAME_HARVEST_V1.json"
