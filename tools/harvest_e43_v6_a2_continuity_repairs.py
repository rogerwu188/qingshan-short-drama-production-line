#!/usr/bin/env python3
"""Query and download the five already-bound E43 A2 repair tasks; never POST."""
...
SUBMISSION = ROOT / "qa/e43_v6_a2_continuity_repairs/E43_V6_A2_GIGGLE_VIDEO_SUBMISSION_V1.json"
ASSET_DIR = ROOT / "working_assets/e43_v6_video_units_a2_continuity_repairs"
RAW_DIR = ROOT / "qa/e43_v6_a2_continuity_repairs/raw_status"
OUT = ROOT / "qa/e43_v6_a2_continuity_repairs/E43_V6_A2_VIDEO_HARVEST_LATEST.json"
...
def one(task: dict, api_key: str) -> dict:
    ...
    unit_id = task_key.split("-VIDEO-")[0]
...
def main() -> int:
    ...
    if submission.get("status") != "PASS" or submission.get("submitted") != 5 or submission.get("failed") != 0:
        raise ValueError("E43 A2 submission is not the expected 5/5 bound batch")
    if len(tasks) != 5 or len({row.get("task_id") for row in tasks}) != 5:
        raise ValueError("E43 A2 harvest does not bind exactly five unique task IDs")
    ...
    payload = {
        "schema": "qingshan.e43.v6.a2_continuity_repair_harvest.v1",
        "episode": "E43",
        "observed_at": now(),
