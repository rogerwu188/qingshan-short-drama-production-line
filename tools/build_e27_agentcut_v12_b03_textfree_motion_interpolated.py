#!/usr/bin/env python3
"""Build E27 B03 P2 from text-free intervals with motion interpolation."""
...
BASE = ROOT / "configs/e27_agentcut_project_v11_b03_textfree_segments_20260720.json"
OUT = ROOT / "configs/e27_agentcut_project_v12_b03_textfree_motion_interpolated_20260720.json"
...
REPAIRED = ROOT / "working_assets/e27_b03_p2_textfree_visual_v4_20260720/E27_B03_P2_TEXTFREE_VISUAL_V4.mp4"
...
def main() -> None:
    ...
    subprocess.run([
        ...
        (
            ...
            f"[1:v]trim=start=0:end=2,setpts=1.25*(PTS-STARTPTS),{interpolate}[r0];"
            f"[1:v]trim=start=5:end=7,setpts=1.25*(PTS-STARTPTS),{interpolate}[r1];"
            f"[1:v]trim=start=10:end=12,setpts=1.25*(PTS-STARTPTS),{interpolate}[r2];"
    ...
    project["metadata"].update({
        "status": "AGENTCUT_V12_B03_TEXTFREE_MOTION_INTERPOLATED_NOT_FINAL",
        "source_project": str(BASE),
        "rollback_from": str(BASE.relative_to(ROOT)),
        "change_scope": (
            "Replace V11 duplicate-frame time extension with motion-interpolated "
            "text-free intervals; preserve original dialogue audio and runtime."
        ),
    })
    project["output"]["path"] = str(
        ROOT / "exports/e27/agentcut_v12_b03_textfree_motion_interpolated_20260720/E27_AGENTCUT_V12_B03_TEXTFREE_MOTION_INTERPOLATED_NOT_FINAL.mp4"
    )
    changed = 0
    for clip in project["timeline"]["videoTracks"][0]["clips"]:
        ...
        repair.update({
            "kind": "VERIFIED_TEXT_FREE_INTERVAL_REPLACEMENT_V4_MOTION_INTERPOLATED",
            "motion_interpolation": interpolate,
            "v11_failure": "periodic_duplicate_cadence_detected:frames=1537-1726:events=39:interval=5",
