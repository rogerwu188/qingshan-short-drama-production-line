#!/usr/bin/env python3
"""Generate and machine-QA release-clear local U07 DIA-006 audio candidates."""
...
OUT = ROOT / "working_assets/e40_production_20260814/u07_v2_kokoro_exact_audio_candidates_v1"
RECEIPT = ROOT / "workflow/tasks/E40_U07_V2_KOKORO_EXACT_AUDIO_CANDIDATE_GENERATION_20260814.json"
...
QA = ROOT / "qa/e40_production_20260814/u07_v2_kokoro_exact_audio_candidates_v1/E40_U07_V2_KOKORO_EXACT_AUDIO_MACHINE_QA_V1.json"
...
def generate() -> int:
    ...
    rights = {
        ...
        "provider_credits": 0,
        "continuity_basis": "Preserve admitted U05-U06 Chenji voice identity with zm_009.",
        "provider_posts": 0,
    }
    ...
    for voice in VOICES:
        ...
        for speed in SPEEDS:
            ...
            source = OUT / f"E40-DIA006_{voice}_speed{tag}_source24k.wav"
            normalized = OUT / f"E40-DIA006_{voice}_speed{tag}_normalized48k.wav"
            sf.write(source, result.audio.detach().cpu().numpy(), 24000)
            subprocess.run(
                [FFMPEG, "-y", "-v", "error", "-i", str(source), "-af", "loudnorm=I=-18:TP=-2:LRA=7", "-ar", "48000", "-ac", "1", "-c:a", "pcm_s16le", str(normalized)],
                check=True,
            )
            outputs.append(
                {
                    ...
                    "voice_file_sha256": sha(voice_paths[voice]),
                }
            )
    atomic_json(
        RECEIPT,
        {
            "schema": "qingshan.e40.u07.v2.kokoro_candidate_generation.v1",
    ...
    print(json.dumps({"status": "PASS_ZERO_CREDIT_CANDIDATES_GENERATED_QA_PENDING", "candidates": len(outputs)}, ensure_ascii=False))
    return 0
...
def qa() -> int:
    ...
    for item in generated:
        ...
        segments, _ = model.transcribe(
            str(path), language="zh", vad_filter=True, beam_size=5, initial_prompt="以下是简体中文普通话对白。", hotwords=TEXT
        )
    ...
    passes.sort(key=lambda row: (abs(row["audio_metrics"]["duration_seconds"] - 3.0), row["speed"]))
    selected = passes[0] if passes else None
    payload = {
        "schema": "qingshan.e40.u07.v2.kokoro_exact_audio_machine_qa.v1",
        ...
        "candidates": rows,
        "selected": selected,
        "selection_rule": "exact ASR and audio gates; preserve admitted zm_009 identity; duration nearest 3.0s",
        "provider_posts": 0,
        "credits": 0,
    }
    atomic_json(QA, payload)
