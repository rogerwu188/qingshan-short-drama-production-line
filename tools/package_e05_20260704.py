...
RAW = BASE / "exports/e05/qingshan_E05_platform_raw_20260704.mp4"
LOGO = BASE / "libraries/brand/nalu_motion_cat_logo_v1.png"
OUT_DIR = BASE / "exports/e05/final_package_20260704"
QA_DIR = BASE / "qa/e05_final_20260704/final_package"

TITLE_OVERLAY = OUT_DIR / "qingshan_E05_title_overlay_20260704.png"
TAIL_OVERLAY = OUT_DIR / "qingshan_E05_nalu_tail_overlay_20260704.png"
FINAL = OUT_DIR / "qingshan_E05_final_titled_subtitled_nalu_20260704.mp4"

FONT_REGULAR = "/System/Library/Fonts/STHeiti Light.ttc"
FONT_BOLD = "/System/Library/Fonts/STHeiti Medium.ttc"


PLANNED_DURATIONS = [5, 6, 6, 7, 6, 6, 6, 7, 7, 7, 7, 7, 8, 7, 7, 8, 6, 6, 8, 10]
...
SUBTITLES = [
    ...
    "他的情报，不如我值钱。",
    "你很想活？",
    "我没有情报。但给我两刻钟，我把情报找出来。",
    "一个医馆小学徒，能知道什么？",
    "他不过是送药的。",
    "送药的人会进厨房，厨房里，才最容易藏东西。",
    "两刻钟太久。",
    "一片叶子，就是一条命。",
    "成交。但我只给你一刻钟。",
    "为什么……有两罐盐？",
...
def wrap_cn(text: str, max_chars: int = 16) -> str:
    if len(text) <= max_chars:
        return text
    if "，" in text and len(text) <= max_chars * 2:
...
def make_overlays() -> None:
    ...
    draw.rounded_rectangle((52, 190, 668, 552), radius=18, fill=(0, 0, 0, 158))
    draw_center(draw, 246, "青山", ImageFont.truetype(FONT_BOLD, 88), (255, 255, 255, 248))
    draw_center(draw, 372, "第5集：李青鸟的梦门", ImageFont.truetype(FONT_BOLD, 34), (238, 238, 236, 242))
    draw.line((120, 448, 600, 448), fill=(176, 210, 204, 150), width=2)
    draw_center(draw, 482, "梦门一开，陈迹醒在灭门现场。", ImageFont.truetype(FONT_REGULAR, 25), (218, 226, 224, 232))
    ...
    draw_center(draw, 805, "下一集：一刻钟破局", ImageFont.truetype(FONT_BOLD, 34), (238, 238, 238, 235))
...
def make_final() -> None:
    ...
    text_dir = OUT_DIR / "subtitle_text_e05_20260704"
    text_dir.mkdir(parents=True, exist_ok=True)

    draw_filters: list[str] = []
    cursor = 0.0
    for idx, (planned, text) in enumerate(zip(PLANNED_DURATIONS, SUBTITLES), start=1):
        start = cursor * scale + 0.45
        end = min((cursor + planned) * scale - 0.35, duration - 0.2)
        cursor += planned
...
def extract_qa() -> None:
    ...
    times = [0.7, 3.0, 8.8, 24.0, 48.0, 72.0, 96.0, 120.0, max(0.0, duration - 1.8)]
    frames: list[Path] = []
    ...
    for frame in frames:
        ...
        draw.text((8, 324), frame.name, font=ImageFont.truetype(FONT_REGULAR, 15), fill=(230, 230, 230))
        thumbs.append(thumb)
    ...
    sheet.save(QA_DIR / "contact_sheet_e05_final.jpg", quality=92)

    (QA_DIR / "ffmpeg_probe_e05_final.txt").write_text(probe_text(FINAL), encoding="utf-8")
    silence = subprocess.run([
        str(FFMPEG), "-hide_banner", "-i", str(FINAL),
        "-af", "silencedetect=n=-35dB:d=2",
        "-f", "null", "-",
    ], text=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    (QA_DIR / "silencedetect_e05_final.txt").write_text(silence.stdout or "", encoding="utf-8")
...
def main() -> None:
    ...
    extract_qa()
    print(f"FINAL={FINAL}")
    print(f"QA={QA_DIR / 'contact_sheet_e05_final.jpg'}")
