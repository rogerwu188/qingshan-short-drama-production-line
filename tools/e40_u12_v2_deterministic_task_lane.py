#!/usr/bin/env python3
"""CAS-register and close the zero-cost E40 U12 V2 deterministic remediation QA."""
...
TASK_ID = "E40-U12-V2-CHANGED-REPRESENTATION-DETERMINISTIC-PAPER-TRANSFER-REMEDIATION"
...
def main() -> int:
    ...
    if current is None:
        if args.action != "register":
            raise SystemExit("deterministic remediation task is not registered")
        current = {
            "task_id": TASK_ID,
            "lane_id": "QA",
            "state": "QA",
            "zero_cost": True,
            "deliverable_type": "DETERMINISTIC_PAPER_TRANSFER_TOOLCHAIN_MASK_DEPTH_TRAJECTORY_LOCAL_TEST_AND_NO_SUBMIT_RECEIPT",
            "priority": 100,
            "scope": ["E40", "U12", "DETERMINISTIC_COMPOSITING", "MASK", "DEPTH", "TRAJECTORY", "NO_SUBMIT"],
            "exact_predecessor_task_id": "E40-U12-MOUTH-NONVISIBLE-FAST720-SILENT-VISUAL-EXACTLY-ONE",
    ...
    if args.action in {"register", "renew"}:
        current.update({
            "state": "QA",
            "liveness_role": "PRODUCING",
            "lease_owner": "codex-e40-u12:v2-deterministic-paper-transfer-remediation",
    ...
    result = commit_task_updates(
        ...
        writer_id="codex-e40-u12:v2-deterministic-paper-transfer-remediation",
    )
    print(result)
    return 0
