#!/usr/bin/env python3
"""Canonical and substitution-negative matrix for V54."""
from __future__ import annotations
import argparse,hashlib,json,subprocess,sys
from datetime import datetime,timezone
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1];INVOKER=ROOT/'tools/run_e40_u12_v54_pinned_chain_integrity_gate.py'
...
def main():
 ...
 ok=run.returncode==0 and gate.get('status')=='PASS_10_OF_10_PINNED_CHAIN_INVENTORY_BINDINGS_EXACT_NO_MUTATION' and gate.get('binding_pass_count')==10 and gate.get('new_evidence_synthesized') is False and gate.get('authority_keys_admitted')==0 and gate.get('production_assets_admitted')==0 and gate.get('authorization') is False
