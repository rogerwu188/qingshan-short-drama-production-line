...
提速只允许消除重复劳动，不得削弱地图、人物、服装、道具、声音、转场、付费事务和真实媒体边界门禁。

- 付费提交前必须运行 `tools/production_efficiency_contract.py`；失败不得进入远端生成。
- 角色卡、整集地图、逐镜子空间图、服装卡和场景锚点按精确 SHA 跨集复用。只有新人物、新空间、新服装状态、不可插值的物理终态或关键转场终态才生成新关键帧。
- MiniMax-H3 有对白单元默认 6–10 秒且最多两句；无对白连续动作单元可到 15 秒。超限必须记录具名 `efficiency_exception_reason`。
- 每波最多六个视频单元；任一远端任务完成后立即收割并开始技术 QA 与边界预检，不等待整波完成。
- 完成任务按模型、时长、画幅、分辨率、prompt SHA 和全部参考媒体 SHA 建立稳定缓存；缓存命中禁止重复 POST。
- 本地发行编码优先 `h264_videotoolbox`，不可用时回退 `libx264`；归一化分段按内容哈希缓存，最终只做一次字幕、片尾和 2K 合成。

v1.8 smoke test：

```text
输入：一份含 14 个 H3 单元的 grouped manifest，其中有跨集复用锚点、两句以内对白单元和无对白动作单元。
期望：
1. 只为语义新增状态生成关键帧，复用项保留精确 SHA。
2. 合同拒绝超过 10 秒或超过两句且没有具名例外的 H3 对白单元。
3. 输出 6/6/2 三个滚动波次，已完成 handle 立即进入收割和技术 QA。
4. 相同模型、prompt 与参考媒体命中同一缓存键，不得重复提交。
5. 发行机存在 VideoToolbox 时选择硬件 H.264，否则确定性回退 libx264。
6. 所有原有质量门和付费事务门保持启用。
```
