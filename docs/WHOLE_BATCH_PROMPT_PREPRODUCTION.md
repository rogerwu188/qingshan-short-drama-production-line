...
Use `compile_model_prompt(unit, preproduction_only=True)` for this planning stage. It keeps unobserved prop state marked as unobserved and records a non-admission compile receipt; only the unavailable first-frame visual check is deferred. All prop ownership and structural checks still run. Normal compilation and paid validation use the default strict mode, independent of task flags, and reject unverified first-frame states. This avoids a circular dependency on images that cannot yet be generated.
